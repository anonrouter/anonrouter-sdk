...
// module answers the other half: "is the AnonRouter data plane I am talking to
// right now the exact reviewed build running inside an Intel TDX confidential VM,
// and is the key protecting this connection the one that TD holds?".
//
// The binding is the single object whose SHA-512 digest is placed in the TDX
// quote's 64-byte report_data field. Because the TD cannot forge report_data, a
// caller that recomputes this digest from the returned fields and finds it equal
// to the quote's report_data has proven that the attesting TD asserted every
// field at quote time: the caller's fresh nonce, the app/instance identity, the
// compose measurement, the release identity, the public origin, and the
// application public key.
//
// Invariants:
//   - PURE. No I/O, no clock. This file is a faithful port of the gateway's own
//     producer module, so the two can never drift into different digests. The
//     shared KAT vectors (shared/vectors/gateway-binding.json) pin that.
...
//   - Browser-safe. Uint8Array and @noble/hashes only, never node:crypto or
//     Buffer, so it runs unchanged in a browser, Electron, or Node.
//   - Fail closed. Every field is validated; a malformed field throws rather
//     than producing a digest over partially-checked input.

import { sha512 } from "@noble/hashes/sha2.js";
import { bytesToHex, utf8Encode } from "../bytes.js";
 ...
 *                this TD and never left it. `tls_spki_sha256` names that
 ...
 *                Clients that require transport binding must fail closed here and
 *                use the application key for an inner encrypted channel.
 *
...
export interface GatewayAttestationBinding {
   ...
   */
  release_id: string;
  /** The public origin a client must have connected to. */
  origin: string;
   ...
   * lowercase hex, or null when `transport` is "gateway-tls" and the TD does not
   * own the served certificate. Never omitted; null is an assertion, not an
   * absence.
   */
...
export function assertGatewayNonce(nonce: unknown): string {
  ...
  return requireLowercaseHex("nonce", nonce.toLowerCase(), GATEWAY_NONCE_HEX_LENGTH);
}

/**
 * Validate and normalize an untrusted binding into the canonical shape. A field
 * the verifier cannot validate can never contribute to a digest it accepts.
 */
export function normalizeGatewayBinding(input: unknown): GatewayAttestationBinding {
  ...
  return {
    ...
    app_id: requireLowercaseHex(
      "app_id",
      typeof raw.app_id === "string" ? raw.app_id.toLowerCase().replace(/^0x/, "") : raw.app_id
    ),
 ...
 * field order and JSON.stringify's standard string escaping, so any language that
 * can produce the same ordered JSON reproduces the same bytes.
 */
...
export function gatewayBindingDigest(binding: GatewayAttestationBinding): Uint8Array {
  return sha512(utf8Encode(canonicalGatewayBindingJson(binding)));
}
...
export function gatewayBindingHash(binding: GatewayAttestationBinding): string {
  return bytesToHex(gatewayBindingDigest(binding));
}
