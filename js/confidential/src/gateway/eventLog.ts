...
//
// Implemented with @noble/hashes so a verifier does not have to trust, or even
// install, the vendor SDK to check the vendor's own claim, and so this runs
// unchanged in a browser.

import { sha384 } from "@noble/hashes/sha2.js";
import { bytesToHex, concatBytes, hexToBytes, utf8Decode, utf8Encode } from "../bytes.js";
...
export function replayRegister(digests: readonly string[]): string {
  let register = hexToBytes(RTMR_INITIAL_VALUE);
  for (const digest of digests) {
    let content = hexToBytes(digest);
    if (content.length < DIGEST_BYTES) {
      const padded = new Uint8Array(DIGEST_BYTES);
      padded.set(content, 0);
      content = padded;
    }
    register = sha384(concatBytes(register, content));
  }
  return bytesToHex(register);
}
...
export function computeRtmr3EventDigestV1(event: DstackEventLogEntry): string {
  ...
  new DataView(type.buffer).setUint32(0, event.event_type >>> 0, true);
  const colon = utf8Encode(":");
  return bytesToHex(sha384(concatBytes(
    type,
    colon,
    utf8Encode(event.event),
    colon,
...
function v2DigestIsSelfConsistent(event: DstackEventLogEntry): boolean {
  ...
  if (bytesToHex(sha384(bytes)) !== event.digest) return false;
 ...
 * returns RTMR3 entries with an EMPTY `digest` and expects the verifier to derive
 * it. That is the stronger arrangement, and it is why this returns the computed
 * value rather than comparing against a supplied one: when the replay is
 * performed over digests derived from (event_type, event, event_payload),
 ...
 * from digests DERIVED from each event's own fields, because that is what the
 * guest agent expects (it ships those entries with an empty digest) and because
 * it is what makes reproducing the register prove the payloads.
 *
 * An RTMR3 entry that cannot be hashed contributes a value that cannot match, so
 * the replay fails rather than silently skipping it.
 */
...
/**
 * Read exactly one named RTMR3 event's payload from the log, ONLY if that event's
 * digest self-consistently commits to the payload. Returns null when the event is
 * absent. Throws when it appears more than once (a duplicated "compose-hash"
 * would let a server present whichever value the reader picks first) or when the
