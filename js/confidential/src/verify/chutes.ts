...
export class ChutesTeeVerifier implements TeeVerifier {
  ...
  verifyAttestation(evidence: unknown, expectations: AttestationExpectations): NormalizedAttestationResult {
    ...
    // `advisoryGaps` instead of having to know the format to infer it.
    //
    // What still binds the model on this route is AnonRouter's own attested
    // relay echoing the route back, which the two-hop verdict cross-checks. That
    // is a weaker statement than the provider's enclave naming its own weights,
    // and the two must not be read as the same thing.
    checks.push(check("model_binding", false, false,
